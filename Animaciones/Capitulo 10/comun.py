...
def linea_datos(ax, xs, ys, color=AZUL, ancho=3.0, suave=False):
    ...
    # OJO: para atenuar una curva usar set_stroke(opacity=...), NO
    # set_opacity(), que toca tambien el relleno y la convierte en mancha
    m = VMobject(color=color, stroke_width=ancho, fill_opacity=0)
    if suave:
